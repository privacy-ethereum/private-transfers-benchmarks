...
- **Generalised smart-contract rollups** (any L2/L3 where arbitrary contracts can be deployed — Aztec, Arbitrum, Base, zkSync, Scroll, …) → "Unlimited access to DeFi applications". The rollup IS the DeFi surface: any DeFi app can be deployed inside it, so the protocol's own shielded assets can compose with the full set. The fact that external L1 DeFi is only reachable via async bridging does NOT downgrade the value — that constraint applies to every L2.
