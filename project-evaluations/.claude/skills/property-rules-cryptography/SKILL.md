...
**Trusted hardware** — a TEE / SGX enclave performs a validity job whose correctness you trust to hardware attestation: executing the transfer, or verifying the proofs and signed messages a contract accepts (mirage, redact). A TEE used only to decrypt or protect data in transit is confidentiality, not validity — do NOT tag it (fluton). When a protocol's own docs disagree on the enclave's role — one page framing it as key-share confidentiality, another tying it to computation integrity ("the enclave ensures nodes run the correct software", "verifiability depends on hardware security") — defer to the more architectural / integrity-claiming source and tag Trusted hardware (zama: the litepaper roots MPC integrity in AWS Nitro Enclaves even though the KMS component page frames them as key protection). Do not let the narrower confidentiality framing override an explicit integrity claim.
