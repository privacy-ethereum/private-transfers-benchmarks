...
When such a rollup or validium has its own settlement step but no published batch or epoch cadence — a permissioned validium whose operator configures the interval privately, for example — do not invent a seconds figure and do not emit INSUFFICIENT_DATA. Use "Underlying chain" as the conservative value and set `needsResearchReview` naming the missing cadence (e.g. "batch cadence operator-configurable and unpublished, so a seconds-to-L1-verification figure is not knowable"). Describe such a chain as an L2/L3 that settles to its host chain, never as an L1.
