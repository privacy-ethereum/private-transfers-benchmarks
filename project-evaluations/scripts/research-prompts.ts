...
22. Notes describe the protocol's actual configuration, not the grading exercise. Never write meta-commentary about how the value was chosen — forbidden: "value assumes the achievable best case", "the value reflects the best case", "this value assumes", "assuming the user", "in the best case". When VALUE_FORMAT_RULES #5 (possible-behaviour) applies, state the self-hosted configuration plainly as a fact (e.g. "a user who runs their own indexer sees no third-party leakage") and describe the more-exposed default — do not frame the grade as resting on an assumption.`;
...
15. Numeric value must be cited on its own property. When a property's value IS a number — a finality/withdrawal duration in seconds, a challenge-window length, a timelock, a tree depth, an anonymity-set size — the load-bearing figure (or the human-readable form it converts from, e.g. "~18 days" for 1,555,200 seconds) must appear inside one of THAT property's own cited_text spans, not only on a sibling property or in the research summary. A timing property whose only citation proves "finality is challenge-gated" but not the actual window length leaves the number unsubstantiated. Re-cite the span carrying the figure onto the property, or set \`needsResearchReview\` naming the uncited number.
16. Subject-attribution check. A figure, capability, or property stated in a citation's \`cited_text\` must be ascribed by the source to THIS protocol — not to prior art, a competitor, generic tooling, or a motivating example the source contrasts against. When the cited span attributes the fact to a different subject (e.g. "most zk tech at the time took five minutes to prove on a phone", "unlike other rollups"), the notes must not restate it as a fact about this protocol. Flag and reword to what the source says about this protocol specifically, or drop the claim.`;
