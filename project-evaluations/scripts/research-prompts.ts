...
8. Censorship resistance ⇐ enforcement mechanisms. If any entity can stop a valid transfer or withdrawal — an asset-level blocklist/allowlist hook that reverts on listed addresses, a verifier or circuit an admin can upgrade or brick, or a permissioned executor set with no permissionless fallback path for the user — then Censorship resistance = No, regardless of how permissionless the underlying chain is. Combine with rule 5: if a blocklist hook makes Enforcement entities ≠ None, it almost certainly makes Censorship resistance = No as well. Privacy-pool verifier-upgrade mechanical check: for smart-contract privacy protocols (shielded pool, stealth-address aggregator, mixer, ZK transfer protocol, etc.) where the on-chain transaction verifier sits inside an upgradeable proxy and the admin is empowered to swap or brick the implementation — Upgradeability ∈ {Single admin, Multi-sig, DAO} — Censorship resistance is No, because a verifier swap blocks all future transfers and unwrap paths. Exception: when the protocol's docs explicitly limit admin authority to non-essential parameters (fee tolls, treasury settings) with no verifier-swap or implementation-upgrade power, the rule doesn't apply and Censorship resistance can be Yes provided the protocol is otherwise permissionless.
