...
19. Length. Target 60-100 words per property; never exceed 120 words (~700 chars). If your notes are running long, you are almost certainly explaining mechanism rather than answering the property — mechanism belongs in the protocol's top-level description, not 30 times across properties. Trim until every sentence is load-bearing for the chosen value or a boundary caveat that could flip the verdict.
...
4. Do not emit INSUFFICIENT_DATA when a sensible enum default exists for the protocol type. Application-layer protocols deployed on existing chains use N/A for Time-to-finality (finality inherited from the host chain); they use 0 for Deposit time / Withdraw time when no protocol-imposed queue, challenge window, or time-lock is documented (host-chain block latency is not a waiting period). For Escape hatch and Upgradeability where source is not public, pick the conservative enum (No / Single admin) and set \`needsResearchReview\` to a reason string naming the missing source — INSUFFICIENT_DATA is reserved for genuinely unanswerable cases, not "the closed-source pick is uncertain".
`;
